...
:meth:`~wand.image.Image.rotate()`.  It takes a ``degree`` which can be 0 to
359.  (Actually you can pass 360, 361, or more but it will be the same to
...
As like :meth:`~wand.image.Image.flip()`, :meth:`~wand.image.Image.flop()` does
the same thing except it doesn't make a vertical mirror image but horizontal::
